...
class DynamicLoading{
            ...
            if(lists[i].length>0)
                for(var j=0;j<lists[i].length&&j<4000;j++)//for(var j in lists[i])
                    if(window.meshes[""+lists[i][j]]){
                        ...
                        // console.log(window.meshes[""+lists[i][j]])
                    }
        // var test000=0
        // for(var i in window.meshes)
    ...
    getList(min,step,max) {//用于渲染
        ...
        function add(direct_i,maxNumber){
            ...
            for(var j=0;j<list.length&&j<maxNumber;j++){
                ...
                for(var k=0;k<group.length;k++){
                    var meshIndex=group[k]
                    // if(list_all.length<100||Math.random()<100/(5*list_all.length))
                        list_all.push(group[k])
                    // if(list_all.length<200&&!window.loadedLZC[meshIndex]){
                    //     window.loadedLZC[meshIndex]=1;
                    //     list_all.push(group[k])
                    // }
        ...
        // for(var i=0;i<this.visualList.length;i++){//6个方向
        //     // var coefficient=0.5
        //     // if (i==direct.max)coefficient=1
            
        //     var list=this.visualList[direct[i]][xi][yi][zi]
        //     for(var j=0;j<list.length;j++){
        //         var group_id=list[j]
        //         var group=this.groups_arr[group_id]
        //         for(var k=0;k<group.length;k++){
        //             if(list_all.length<5000)
        //                 list_all.push(group[k])
        //             // else
        //                 // break;//return list_all
        //         }
        //     }
        //     //list_all.push(list)
        // }
