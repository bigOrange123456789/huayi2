...
function PlayerControl0(camera){
    ...
    myMouseManager.dragMouse=function (dx,dy) {
        ...
        if(myMouseManager.button===0){//左键拖动
            scope.rotation1(-0.0015*dx);
            scope.rotation2(-0.0015*dy);
        }else if(myMouseManager.button===1){//滚轮拖动--不是滑轮滚动
            scope.move(0,0.015*dy,0.015*dx);
        }else{//右键拖动
            scope.move(0,0.5*dy,0.5*dx);
        }
    }
    ...
    myPhoneManager.drag=function(dx,dy){
        if(!scope.ableRotation)return;
        scope.rotation1(-0.05*dx);
        scope.rotation2(-0.05*dy);
    }
